...
class OrderTakingScreen extends StatelessWidget {
                      ...
                      if (state is PosError) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            ...
                            margin: const EdgeInsets.only(top: 20, left: 400, right: 24, bottom: 0),
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
                            content: Row(
                              children: [
                                const Icon(Icons.error_outline, color: Colors.white),
                                const SizedBox(width: 12),
                                Expanded(child: Text(state.message, style: const TextStyle(fontWeight: FontWeight.bold))),
                              ],
                            ),
                          ),
                        );
                          ...
                          child: ListView(
                            ...
                            children: [
                              ...
                              if (persistedItems.isNotEmpty) ...[
                                ...
                                ...persistedItems.map((item) => _buildTicketItem(context, item, state, isPersisted: true, orderId: activeOrder?.id)),
                              ],
                              ...
                              if (draftItems.isNotEmpty) ...[
                                ...
                                ...draftItems.asMap().entries.map((entry) => _buildTicketItem(context, entry.value, state, isPersisted: false, index: entry.key)),
                              ],
                                ...
                                children: [
                                  Expanded(
                                    child: OutlinedButton(
                                                ...
                                                SnackBar(
                                                  ...
                                                  content: Row(
                                                    children: [
                                                      ...
                                                      const Expanded(
                                                        child: Text('¡Comanda enviada! 🚀', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 14)),
                                                      ),
                                                    ],
                                                  ),
                                                ),
                                      ...
                                      child: Text('PEDIR', style: TextStyle(color: draftItems.isNotEmpty ? const Color(0xFF3B82F6) : Colors.white10, fontWeight: FontWeight.w900, fontSize: 13, letterSpacing: 1.5)),
                                    ),
                                  ),
                                  const SizedBox(width: 12),
                                  Expanded(
  ...
  Widget _buildTicketItem(BuildContext context, dynamic item, PosLoaded state, {required bool isPersisted, int? index, String? orderId}) {
    final categories = state.categories;
    final products = state.products;
    final bool isVoided = isPersisted && item.isVoided;
    final totalItemPrice = item.unitPriceSnapshot * item.quantity;
    
    // Resolve note based on model type
    final String? displayNote = (item is OrderItemModel) ? item.notes : (item.note as String?);
    
    String? categoryId = item.product?.categoryId;
    if (categoryId == null) {
      final product = products.firstWhere((p) => p.id == item.productId, orElse: () => ProductModel(id: '', name: '', price: 0, categoryId: '', isActive: true));
      categoryId = product.categoryId;
    }
    
    final category = categories.firstWhere((c) => c.id == categoryId, orElse: () => CategoryModel(id: '', name: 'N/A'));
    
    // Find production sector name
    String sectorName = 'SIN SECTOR';
    if (category.productionSectorId != null) {
      final pSector = state.productionSectors.firstWhere((s) => s.id == category.productionSectorId, orElse: () => ProductionSectorModel(id: '', name: ''));
      if (pSector.name.isNotEmpty) sectorName = pSector.name;
    }
    
    final sectorColor = _parseColor(category.color);
          ...
          children: [
            Row(
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        ...
                        style: TextStyle(
                          color: isVoided ? Colors.white24 : Colors.white,
                      ...
                      Row(
                        children: [
                          ...
                          Text(
                            ...
                            style: TextStyle(color: sectorColor.withOpacity(0.7), fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 0.5),
                          ),
                        ],
                      ),
                    ],
                ...
                // Note Button (Visible for draft items)
                if (!isPersisted && index != null)
                  IconButton(
                    icon: const Icon(Icons.edit_note_rounded, color: Color(0xFF3B82F6), size: 24),
                    onPressed: () => _showNoteDialog(context, item, index),
                    tooltip: 'Añadir Nota',
                ...
                Column(
                  ...
                  children: [
                    Text(
                      ...
                      style: TextStyle(
                        color: isVoided ? Colors.white10 : Colors.white,
                        fontSize: 13,
                        fontWeight: FontWeight.bold,
                        decoration: isVoided ? TextDecoration.lineThrough : null,
                      ),
                    ...
                    if (!isPersisted && index != null)
                      Padding(
                        ...
                        child: InkWell(
                          onTap: () => context.read<PosBloc>().add(PosDraftItemRemoved(table.id, index)),
                          child: Icon(Icons.remove_circle_outline, color: Colors.redAccent.withOpacity(0.7), size: 20),
                        ),
                      ),
                    if (isPersisted && !isVoided && orderId != null)
                      InkWell(
                        onTap: () => context.read<PosBloc>().add(PosItemVoided(orderId, item.id)),
                        child: const Padding(
                          padding: EdgeInsets.only(top: 6),
                          child: Text('Anular', style: TextStyle(color: Colors.redAccent, fontSize: 11, fontWeight: FontWeight.bold)),
                        ),
                      ),
                    if (isVoided)
                      const Padding(
                        padding: EdgeInsets.only(top: 4),
                        child: Text('ANULADO', style: TextStyle(color: Colors.red, fontSize: 9, fontWeight: FontWeight.bold)),
                      ),
                  ],
                ),
              ],
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Flexible(
                      child: Text(
                        displayNote,
                        style: const TextStyle(color: Color(0xFF3B82F6), fontSize: 12, fontStyle: FontStyle.italic),
                      ),
                    ),
                  ],
                ),
  ...
  void _showNoteDialog(BuildContext context, dynamic item, int index) {
    final String? currentNote = (item is OrderItemModel) ? item.notes : (item.note as String?);
